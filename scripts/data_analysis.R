...
# stating new save data frame name & required data set
  group_by(count_type, habitat) |> 
# stating which columns to include in analysis, & order on table
  summarise(mean = mean(g5_lampyridae, na.rm =T), 
# calculating means using values from g5_lampyridae column
            sd = sd(g5_lampyridae, na.rm=T)) 
# calculating standard deviations from g5_lampyridae column
